FROM tomcat:8.5.49-jdk8-openjdk-slim

#Ports
EXPOSE 8080/tcp

#Psql libs
RUN apt-get update && apt-get install -y libpq5 wget && rm -rf /var/lib/apt/lists/*
...
RUN mkdir -p webapps/cmdbuild/ && wget https://sourceforge.net/projects/cmdbuild/files/3.1.1/cmdbuild-3.1.1.war/download -O webapps/cmdbuild/cmdbuild.war
